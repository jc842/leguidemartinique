# Directives Éditoriales & Contraintes d'Environnement — leguidemartinique.com

Ce document récapitule les directives d'environnement et de monétisation obligatoires pour toute création de nouvel article sur **leguidemartinique.com**.
...
| :--- | :--- |
| **Domaine** | `leguidemartinique.com` |
| **Tracker Analytics** | Matomo cookieless (`https://analytics.les4h.fr/`) |
| **Conteneur MTM** | `container_YYjvMVvd.js` (Site ID: `108`) |
| **Adserver Revive** | `https://ads.les4h.fr/` (Éditeur ID: `113`) |
...
```html
<script type="text/javascript" src="https://analytics.les4h.fr/js/container_YYjvMVvd.js" async defer></script>
```
...
<!-- 1. Encart Pub In-Content Obligatoire (Medium Rectangle 300x250 - Zone 679) -->
<div class="my-12 py-6 border-y border-stone-200/80 flex justify-center">
  <ReviveSlot slot="inContent" />
</div>

<!-- 2. Encart Pub Fin d'Article Obligatoire (Large Rectangle 336x280 - Zone 677) -->
...
| :--- | :--- | :---: | :--- |
| **`header`** | Leaderboard (728x90) | **678** | Haut de page / sous navigation |
| **`inContent`** | Medium Rectangle (300x250) | **679** | Milieu d'article contextuel |
| **`largeRectangle`** | Large Rectangle (336x280) | **677** | Fin d'article avant recommandations |
| **`mobileSticky`** | Mobile Banner (320x100) | **680** | Bandeau sticky bas d'écran mobile |
| **`halfPage`** | Half Page (300x600) | **676** | Sidebar desktop pour articles longs |
| **`skyscraper`** | Skyscraper (160x600) | **681** | Colonne latérale de listing / dossiers |

---

## 4. Checklist Avant Publication
- [ ] L'article utilise le Layout avec le tracker MTM actif (`YYjvMVvd`).
